...
<div class="container">
    ...
    <div class="content">
        ...
        <p><strong>{{ $inviterName }}</strong> vás pozýva, aby ste sa pripojili k spoločnosti
            <strong>{{ $companyName }}</strong> ako <strong>{{ $roleLabel }}</strong>.</p>

        @if($hasAccount)
            <p>Prihláste sa a kliknite nižšie pre prijatie:</p>
        @else
            <p>Zaregistrujte sa na NTI portáli ako zástupca spoločnosti — pozvánka sa automaticky prepojí s vaším účtom.</p>
        @endif
